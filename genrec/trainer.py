...
class Trainer:
    ...
    def fit(self, train_dataloader, val_dataloader):
        ...
        for epoch in range(n_epochs):
            ...
            self.log(f'[Epoch {epoch + 1}] Training Time: {train_elapsed_time:.4f}s')
            ...
            if (epoch + 1) % self.config['eval_interval'] == 0:
                ...
                self.log(f'[Epoch {epoch + 1}] Eval Time: {eval_elapsed_time:.4f}s')
        ...
        # Print GPU memory usage at the end of training
        if torch.cuda.is_available():
            gpu_memory_allocated = torch.cuda.max_memory_allocated() / (1024 ** 3)  # Convert to GB
            gpu_memory_reserved = torch.cuda.max_memory_reserved() / (1024 ** 3)  # Convert to GB
            self.log(f'GPU Memory Allocated (Max): {gpu_memory_allocated:.4f} GB')
            self.log(f'GPU Memory Reserved (Max): {gpu_memory_reserved:.4f} GB')
            ...
            with torch.no_grad():
                ...
                if self.config['use_ddp']:  # ddp, gather data from all devices for evaluation
                    ...
                    if isinstance(preds, tuple):
                        ...
                        all_preds, all_labels, all_n_visited_items = self.accelerator.gather_for_metrics(
                            (preds, batch['labels'], n_visited_items))
        ...
        for batch in val_progress_bar:
            with torch.no_grad():
                ...
                states = outputs.final_states.gather(
                    dim=1,
                    index=(batch['seq_lens'] - 1).view(-1, 1, 1, 1).expand(-1, 1, self.model.n_pred_head,
                ...
                logits = [torch.matmul(states[:, 0, i, :], token_embs[i].T) / self.model.temperature for i in
                          range(self.model.n_pred_head)]
